...
class Utils {
	...
	runCommand(command, func = null) {
		if (typeof command !== "string" || (func && typeof func !== "function")) return false;
		if (!this.client || this.client.readyState !== wsOPEN) return false;
		if (new TextEncoder().encode(command).length >= 462) return false;

		return this.runCommandUnsafe(command, func);
	}

	subscribe(event, func = null) {
		if (typeof event !== "string" || typeof func !== "function") return false;
		if (!this.client || this.client.readyState !== wsOPEN) return false;

		Utils.setMulti(this.subscribeBack, event, func);
